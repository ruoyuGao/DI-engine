...
collector_env_num = 1
evaluator_env_num = 1
mobile_ppo_rnd_config = dict(
    exp_name='gym_mobile_rnd_onppo_seed0',
    env=dict(
        ...
        max_step=100,
        stop_value=100000,  # run fixed env_steps
        # stop_value=0.96,
    ),
    reward_model=dict(
...
mobile_ppo_rnd_config = EasyDict(mobile_ppo_rnd_config)
main_config = mobile_ppo_rnd_config
mobile_ppo_rnd_create_config = dict(
    env=dict(
        type='gym_mobile',
        import_names=['dizoo.gym_mobile.envs.gym_mobile_env'],
    ),
    env_manager=dict(type='base'),
    policy=dict(type='ppo'),
    reward_model=dict(type='rnd'),
)
mobile_ppo_rnd_create_config = EasyDict(mobile_ppo_rnd_create_config)
create_config = mobile_ppo_rnd_create_config
